...
#     c: load profile (opt)
    def __init__(self,model_params):
        ...
        # self.electrolyzer_installed_cost_opt=pem_cost.loc[year][pem_cost_cases]
    ...
    def calc_lcoh(self,H2_Results,policy_option,storage_type,solar_size_mw,wind_size_mw,battery_size_mw,battery_hrs,energy_from_renewables_kWh,return_details=False):
        ...
        if return_details:
            # desc_keys = ['Wind [MW]','Solar [MW]','Battery [MW]','Battery [Hrs]']
            ...
            # desc_vals = [wind_size_mw,solar_size_mw,battery_size_mw,battery_hrs]
            ...
            # lcoh_info = pd.Series(dict(zip([desc_keys,gen_cost_keys,lcoh_keys],[desc_vals,gen_cost_vals,lcoh_vals])))
            # lcoh_info = pd.Series(dict(zip([gen_cost_keys,lcoh_keys],[gen_cost_vals,lcoh_vals])))
