...
# lcoe = 4.7
    ...
    ck=['Compressor CapEx [$]','Compressor FOM [frac]','Compressor FOM [$/year]']
    pk=['Pipeline CapEx [$/km]','Pipeline CapEx [$]','Pipeline FOM [frac]','Pipeline FOM [$/year]']
    hk=['H2 Transmission Capacity [kg/day]','Total H2 Transmission Capex [$]']
    cv=[compressor_capex, compressor_FOM_frac, compressor_FOM_USD_yr]

    pv=[pipeline_capex_perkm, pipeline_capex, pipeline_FOM_frac, pipeline_FOM_USD_yr]
    hv=[hydrogen_flow_capacity_kg_day,total_capex]

    all_k = ck + pk + hk
    all_v = cv + pv + hv
    df=pd.Series(dict(zip(all_k,all_v)))
    

    
    return(sol,h2_transmission_price_breakdown,df)
