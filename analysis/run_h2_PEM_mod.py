...
def clean_up_lifetime_performance(h2_tot,use_degradation_penalty):
   ...
   t_eod_metrics = list(h2_tot["Cluster #0"].index.values)
   ...
   for ci,cname in enumerate(cluster_names):
      ...
      for t_eod_type in t_eod_metrics:
         temp_df = pd.DataFrame(h2_tot[cname].loc[t_eod_type].loc[lifetime_desc]).T
   ...
   return cluster_avg_df,redo_performance_df
...
def run_h2_PEM(electrical_generation_timeseries, electrolyzer_size,
   ...
   debug_mode = True #if False, then the outputs are cleaned  up
   ...
   if grid_connection_scenario!='off-grid':
      ...
      if pem_control_type == 'optimize':
         ...
         h2_ts,h2_tot=pem.run_quick() #mod
   h2_res = {}
   ...
   # h2_tot.loc['Average Efficiency [kWh/kg]'].mean()
   # h2_tot.loc['Stack Life [hrs]'].mean()
   # h2_tot.loc['Average Annual Hydrogen Produced [kg]'].sum()
   ...
   # elec_rated_h2_capacity_kgpy =h2_tot.loc['Cluster Rated H2 Production [kg/yr]']*n_pem_clusters
   ...
   # max_h2_pr_hr = h2_tot.loc['Cluster Rated H2 Production [kg/hr]'].iloc[0]*n_pem_clusters
   # max_pwr_pr_hr = h2_tot.loc['Cluster Rated Power Consumed [kWh]'].iloc[0]*n_pem_clusters
   # rated_kWh_pr_kg = h2_tot.loc['Stack Rated Efficiency [kWh/kg]'].iloc[0]#.mean()
   # elec_rated_h2_capacity_kgpy =h2_tot.loc['Cluster Rated H2 Production [kg/yr]'].iloc[0]*n_pem_clusters
   ...
   # new_H2_Results=dict(zip(life_spec_desc,cluster_avg_df[stack_life_spec_desc].values))
